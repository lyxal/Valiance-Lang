...
from valiance.lexer.PrettyPrinter import pretty_print_token, pretty_print_tokens
...
class TestParserPrettyPrinter:
    ...
    def test_pretty_print_variable_set_node(self):
        ...
        node = VariableSetNode(
            name="x",
            value=LiteralNode(value="5", type_=NumberType())
        )
        result = pretty_print_ast(node)
    ...
    def test_pretty_print_if_node(self):
        ...
        node = IfNode(
            condition=GroupNode(elements=[
                LiteralNode(value="true", type_=StringType())
            ]),
            then_branch=GroupNode(elements=[
                LiteralNode(value="42", type_=NumberType())
            ])
        )
        result = pretty_print_ast(node)
    ...
    def test_pretty_print_ast_with_max_depth(self):
        ...
        node = GroupNode(elements=[
            GroupNode(elements=[
                GroupNode(elements=[
                    LiteralNode(value="deep", type_=StringType())
                ])
            ])
        ])
