...
def _format_ast_node(
    ...
    if isinstance(node, GroupNode):
        ...
        for elem in node.elements:
            lines.append(_format_ast_node(elem, indent + 2, max_depth, current_depth + 1))
        ...
        if node.args:
            ...
            for name, arg_node in node.args:
                ...
                lines.append(_format_ast_node(arg_node, indent + 6, max_depth, current_depth + 1))
            lines.append(f"{indent_str}  ]")
        if node.modified:
        ...
        lines.append(_format_ast_node(node.body, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        for obj in node.options:
            lines.append(_format_ast_node(obj, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str}  ]")
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        if node.implemented_traits:
            lines.append(f"{indent_str}  implemented_traits={_format_value(node.implemented_traits)}")
        lines.append(f"{indent_str}  body:")
        lines.append(_format_ast_node(node.body, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        if node.other_traits:
            lines.append(f"{indent_str}  other_traits={_format_value(node.other_traits)}")
        lines.append(f"{indent_str}  body:")
        lines.append(_format_ast_node(node.body, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        for item in node.items:
            lines.append(_format_ast_node(item, indent + 2, max_depth, current_depth + 1))
        lines.append(f"{indent_str}])")
        return "\n".join(lines)
        ...
        lines.append(_format_ast_node(node.value, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        lines.append(_format_ast_node(node.function, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        lines.append(_format_ast_node(node.condition, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str}  then:")
        lines.append(_format_ast_node(node.then_branch, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        lines.append(_format_ast_node(node.else_branch, indent + 4, max_depth, current_depth + 1))
        lines.append(f"{indent_str})")
        return "\n".join(lines)
        ...
        for condition, body in node.branches:
            lines.append(f"{indent_str}    condition:")
            lines.append(_format_ast_node(condition, indent + 6, max_depth, current_depth + 1))
            lines.append(f"{indent_str}    body:")
            lines.append(_format_ast_node(body, indent + 6, max_depth, current_depth + 1))
        lines.append(f"{indent_str}  ]")
        lines.append(f"{indent_str})")
        return "\n".join(lines)
